...
/**
 * Info-sized ellipse in layout units. Height follows the text block; width is
 * a % of the canvas from PARAMS.breakerWidth (capped to the visible page).
 *
 * @param {import('./blob.js').BlobSim} sim
 */
function breakerSize(sim) {
  ...
  const widthPct = clamp(PARAMS.breakerWidth, 1, 100) / 100;
  // Full canvas width in layout units (pre-zoom), then take the configured %.
  const halfW = Math.max(visHalfW * widthPct, 12);
