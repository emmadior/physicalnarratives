 ...
 *   onPerfDegrade?: (info: { fps: number }) => void,
...
export function createBlobEngine(canvas, container, callbacks = {}) {
  ...
  const lowPower = Boolean(callbacks.lowPower);
  const { simLag } = readPerfDebugFlags();
  let pixelRatioCap = lowPower ? LOW_POWER_PIXEL_RATIO : 2;
  ...
  if (callbacks.projects?.length) {
    const count = applyProjectsToParams(callbacks.projects, {
      maxBlobs: callbacks.maxBlobs,
      lowPower,
    });
    console.log("[blob] applied CMS projects to blobs", { count, lowPower });
  } else {
    console.log(
  ...
  if (lowPower && typeof callbacks.maxBlobs === "number" && callbacks.maxBlobs > 0) {
    sim.activeBlobLimit = Math.min(callbacks.maxBlobs, sim.blobs.length);
  }
  ...
  function emitSelectionState() {
    ...
    const loading =
      idx >= 0 &&
      (videoManager.isFullLoading(idx) ||
        (pendingSelectIndex === idx && !videoManager.isFullReady(idx)));
    let loadingDots = { visible: false, x: 0, y: 0 };
    if (loading && blob) {
      const zoom = Math.max(sim.focusZoom || 1, 1);
      const pos = layoutToScreen(
        blob.videoFrameCenterX,
      ...
      loadingDots = { visible: true, x: pos.x, y: pos.y };
    }

    callbacks.onSelectionState?.({
      selectedIndex: idx,
      // Player unlocks once the full file can play — even if CORS blocks
      // the WebGL texture (preview stays in the blob in that case).
      fullVideoReady: idx >= 0 && videoManager.isFullReady(idx),
      fullVideoLoading: loading,
  ...
  function canControlPlayback() {
    ...
    const settled =
      ...
      !sim.focusSettling &&
      sim.paramTargetIndex >= 0;
    return settled && videoManager.isFullReady(idx);
  }
  ...
  function syncUniforms() {
    ...
    const blobCount =
      sim.activeBlobLimit == null
        ? blobs.length
        : Math.min(blobs.length, sim.activeBlobLimit);
  ...
  let frameSkip = lowPower;
  let renderToggle = false;
  let softDegradeArmed = false;
  /** @type {ReturnType<typeof createPerfGuard> | null} */
  let softDegradeGuard = null;

  const perfGuard = createPerfGuard({
    simulateLag: simLag,
    onDegrade: (info) => {
      if (disposed || lowPower) return;
      // Soft pass first: drop retina cost. If still bad, cull blobs in-place
      // (never recreate WebGL — that crashes older Safari).
      if (pixelRatioCap > LOW_POWER_PIXEL_RATIO) {
        pixelRatioCap = LOW_POWER_PIXEL_RATIO;
        frameSkip = true;
        const { width, height } = container.getBoundingClientRect();
        resize(width, height);
        console.warn("[blob] low FPS — capping pixel ratio", info);
        softDegradeArmed = true;
        softDegradeGuard = createPerfGuard({
          simulateLag: simLag,
          warmUpMs: simLag ? 400 : 1500,
          badSamplesNeeded: simLag ? 1 : 2,
          onDegrade: (again) => {
            if (disposed || lowPower) return;
            adaptToLowPower(LOW_POWER_MAX_BLOBS);
            callbacks.onPerfDegrade?.(again);
          },
        });
        return;
      }
      adaptToLowPower(LOW_POWER_MAX_BLOBS);
      callbacks.onPerfDegrade?.(info);
    },
  });

  if (lowPower) perfGuard.disable();
  ...
  function frame(now) {
    ...
    if (frameSkip) {
      ...
      if (!renderToggle) {
        if (softDegradeArmed) softDegradeGuard?.tick(now);
        else perfGuard.tick(now);
        return;
      }
    }
  ...
  return {
    ...
    /**
     * Keep the live WebGL context; freeze and hide blobs past `maxBlobs`.
     * Safer than destroying/recreating the renderer on older Safari.
     * @param {number} [maxBlobs]
     */
    adaptToLowPower(maxBlobs = LOW_POWER_MAX_BLOBS) {
      return adaptToLowPower(maxBlobs);
    },
  };
   ...
   * @param {number} [maxBlobs]
   */
  function adaptToLowPower(maxBlobs = LOW_POWER_MAX_BLOBS) {
    if (disposed) return false;
    const limit = Math.max(1, Math.min(maxBlobs, sim.blobs.length));
    if (
      sim.activeBlobLimit != null &&
      sim.activeBlobLimit <= limit &&
      pixelRatioCap <= LOW_POWER_PIXEL_RATIO
    ) {
      return false;
    }

    if (sim.selectedIndex >= limit || sim.paramTargetIndex >= limit) {
      sim.setFocus(-1);
    }
    ...
    pixelRatioCap = LOW_POWER_PIXEL_RATIO;
    frameSkip = true;
    softDegradeArmed = false;
    softDegradeGuard = null;
    perfGuard.disable();

    videoManager.cullFrom(limit);

    for (let i = limit; i < sim.blobs.length; i++) {
      const blob = sim.blobs[i];
      blob.centerX = 5000 + i * 500;
      blob.centerY = 5000;
    ...
    resize(width, height);
    emitSelectionState();
    console.warn("[blob] adapted to low power (in-place)", { maxBlobs: limit });
    return true;
  }
}
