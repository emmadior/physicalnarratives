...
export function createBlobEngine(canvas, container, callbacks = {}) {
  ...
  if (callbacks.projects?.length) {
    ...
    console.log("[blob] no CMS projects — using default PARAMS.blobs", PARAMS.blobs);
  }
  ...
  function updateVideoFadeIn(dt) {
    ...
    for (let i = 0; i < shaderBlobCount; i++) {
      ...
      videoOpacityArray[i] = bp.videoOpacity * eased;
    }
  }
  ...
  function resize(width, height) {
    ...
    renderer.setPixelRatio(
      Math.min(window.devicePixelRatio || 1, videoManager.slowGpu ? 1.25 : 2),
    );
  ...
  function syncUniforms() {
    ...
    for (let b = 0; b < blobCount; b++) {
      ...
      displaceArray[b] = bp.displaceAmount;
    }
    ...
    for (let i = 0; i < blobCount; i++) {
      for (let j = 0; j < blobCount; j++) {
        pairDest[i * shaderBlobCount + j] = sim.pairMix[sim.pairIndex(i, j)] || 0;
      }
    }
    ...
    if (breakerOn > 0.001) {
      uniforms.uBreaker.value.set(breaker.x * layoutScale, breaker.y * layoutScale, 0);
