...
/**
 * @param {{ sim: import('./blob.js').BlobSim, canvas: HTMLCanvasElement, getLayoutScale: () => number, getFocusZoom?: () => number, hitTest: (x: number, y: number) => number, onFocusChange?: (index: number) => void, onHoverChange?: (index: number) => void, onSelectedBlobClick?: () => void }} ctx
...
export function initSelectionInput({
  ...
  const updateCursor = () => {
    ...
    // No idle hover while a blob is selected / transitioning.
    if (sim.selectedIndex >= 0 || sim.paramTargetIndex >= 0) {
      canvas.style.cursor = "default";
      setHover(-1);
      return;
    }
