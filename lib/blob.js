...
/** Blobs past the live perf limit are frozen and not drawn. */
function isPerfCulled(sim, index) {
  return sim.activeBlobLimit != null && index >= sim.activeBlobLimit;
}
...
export class Blob {
  ...
  get videoPinned() {
    return PARAMS.displaceAmount < 0.01;
  }
...
export class BlobSim {
  constructor() {
    ...
    /** When set, blobs at/after this index are skipped (perf adapt). */
    this.activeBlobLimit = null;
    ...
    this.focusVideoW = 0;
    this.focusVideoH = 0;
    this.focusMobilePortrait = false;
    ...
    /** Smoothed aspect-contain strength while selected (avoids scroll-up snap). */
    this.focusRestContain = 1;
  ...
  beginParamTransition(targetIndex) {
    ...
    resetScroll(this);
    this.focusRestContain = targetIndex >= 0 ? 1 : 0;
  ...
  step(dt) {
    ...
    for (const blob of this.blobs) {
      if (isPerfCulled(this, blob.index)) continue;
      blob.resolveRadii();
      ...
      if (blob.exitMotionBlend != null && blob.exitMotionBlend < 1) {
        ...
        blob.exitMotionBlend = Math.min(
          1,
          blob.exitMotionBlend + dt / blendSec,
        );
    ...
    for (const blob of this.blobs) {
      ...
      if (isFocusSelected(this, blob.index) || (settling && selectedPinned)) {
        ...
        const restTarget =
          settle *
          (1 - clamp(this.scrollProgress / 0.12, 0, 1)) *
          (1 - splitAmt * 0.85);
        // Release containment immediately while scrolling/splitting (same as
        // scroll-down). Ease it back in slowly on the way up so springs can
        // heal the silhouette instead of snapping closed.
        if (restTarget < this.focusRestContain) {
          this.focusRestContain = restTarget;
        } else {
          const healSec = Math.max(PARAMS.breakerHealSeconds, 0.05);
          const step = Math.min(1, dt / healSec);
          this.focusRestContain += (restTarget - this.focusRestContain) * step;
        }
        const restAmt = this.focusRestContain;
        containFocusAspect(blob, restAmt);
        if (restAmt > 0.01) {
  ...
  updatePairMix(dt) {
    const n =
      this.activeBlobLimit == null
        ? this.blobs.length
        : Math.min(this.blobs.length, this.activeBlobLimit);
  ...
  applyInternalForces(t, dt) {
    for (const blob of this.blobs) {
      ...
      const motion =
        (focused ? PARAMS.focusMotionSurvival * (settling ? settle : 1) : 1) *
  ...
  applyInterBlobForces(dt) {
    ...
    for (let bi = 0; bi < blobs.length; bi++) {
      if (isPerfCulled(this, bi)) continue;
      for (let bj = bi + 1; bj < blobs.length; bj++) {
        if (isPerfCulled(this, bj)) continue;
        const a = blobs[bi];
        const b = blobs[bj];
