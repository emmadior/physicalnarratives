...
export class Blob {
  ...
  get videoFrameCenterX() {
    if (this.maxX > this.minX) return (this.minX + this.maxX) * 0.5;
    return this.centerX + this.restCenterX;
  }

  get videoFrameCenterY() {
    if (this.maxY > this.minY) return (this.minY + this.maxY) * 0.5;
    return this.centerY + this.restCenterY;
  }

  get videoFrameHalfW() {
    if (this.maxX > this.minX) {
      return Math.max((this.maxX - this.minX) * 0.5, 1) * PARAMS.videoFitPadding;
    }
  ...
  get videoFrameHalfH() {
    if (this.maxY > this.minY) {
      return Math.max((this.maxY - this.minY) * 0.5, 1) * PARAMS.videoFitPadding;
    }
...
export class BlobSim {
  ...
  step(dt) {
    ...
    for (const blob of this.blobs) {
      ...
      blob.updateDrift(dt, this.view, this.blobs, 1);
      blob.updateCentroid();
    }
  ...
  applyInternalForces(t, dt) {
    for (const blob of this.blobs) {
      ...
      const cohesionK =
        ...
        (1 - splitAmt * 0.92);
      const linkK =
        PARAMS.linkStiffness * (settling ? settle : 1) * (1 - splitAmt * 0.55);
    ...
    for (let bi = 0; bi < blobs.length; bi++) {
      for (let bj = bi + 1; bj < blobs.length; bj++) {
        ...
        if (inContact && PARAMS.contactSeparationBias > 0) {
          ...
          const bias = PARAMS.contactSeparationBias * ramp;
