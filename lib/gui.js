...
export function initGUI({ sim, onRebuildBlob }) {
  ...
  breaker.add(PARAMS, "scrollSmoothSeconds", 0.04, 0.8, 0.01).name("smooth secs");
  breaker.add(PARAMS, "breakerWidthScale", 1, 4, 0.05).name("width scale");
  ...
  PARAMS.blobs.forEach((bp, index) => {
    ...
    folder.add(bp, "displaceAmount", 0, 1, 0.01).name("tearing");
