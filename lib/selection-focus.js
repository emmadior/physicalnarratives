...
export function isFocusSelected(sim, index) {
  return (
    ...
    !sim.paramTransitioning &&
    PARAMS.blobs[index]?.displaceAmount < 0.01
  );
}
...
/**
 * Same stretch-to-silhouette mapping in every state, so selecting a blob
 * does not switch UV mode.
 *
 * @param {import('./blob.js').Blob} blob
 * @param {import('./blob.js').BlobSim} [_sim]
 * @param {number} [_index]
 */
export function getBlendedVideoFrame(blob, _sim, _index) {
  return liveSilhouetteFrame(blob);
}
