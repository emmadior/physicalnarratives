...
/** Portrait focus frame on small viewports (slightly wider than 9:16). */
const MOBILE_FOCUS_ASPECT = 11 / 16;
...
export function computeFocusFrames(blob, videoW, videoH) {
  let vidAspect = videoW / Math.max(videoH, 1);
  // Mobile: force a taller-than-wide focus silhouette; desktop keeps native aspect.
  if (isMobileFocusViewport()) {
    vidAspect = Math.min(vidAspect, MOBILE_FOCUS_ASPECT);
  }
...
function measureTargetZoom(blob, view, target, videoW, videoH) {
  ...
  if (isMobileFocusViewport()) zoom *= 1.2;
...
export function applyFocusVideoAspect(sim, index, videoW, videoH) {
  ...
  const mobilePortrait = isMobileFocusViewport();
  const aspectModeChanged = sim.focusMobilePortrait !== mobilePortrait;
  ...
  if (alreadySettled && !aspectModeChanged) {
  ...
  sim.focusVideoW = videoW;
  sim.focusVideoH = videoH;
  sim.focusMobilePortrait = mobilePortrait;
  sim.focusZoomTo = measureTargetZoom(
    blob,
    sim.view,
    newTarget,
    videoW,
  ...
  if (alreadySettled && aspectModeChanged) {
    // Snap live selected layout to the new mobile/desktop frame.
    applyFocusPose(blob, newTarget);
    blob.seat();
    sim.focusZoom = sim.focusZoomTo;
    sim.focusZoomFrom = sim.focusZoomTo;
    sim.focusRest = captureFocusPose(blob);
    sim.focusMorphFrom = sim.focusRest;
    return;
  }
