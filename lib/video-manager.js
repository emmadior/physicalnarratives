...
export function createVideoManager(container, fallbackTexture, uniforms) {
  ...
  function loadFull(index) {
    ...
    if (slot.full) {
      // In-flight load from a prior select — wait for the same element.
      return waitForMediaReady(slot.full.el).then(() => {
        slot.full.ready = true;
        slot.fullReady = true;
        return slot.full.el;
      });
    }
    ...
    const applySrc = (useCors) => {
      ...
      full.el.muted = true;
      full.el.load();
      // Nudge Chrome to start progressive decode on cold first select.
      const el = full.el;
      el.play()
        .then(() => {
          // Don't leave audio running if the user exited mid-load.
          if (slots[index]?.full?.el !== el) return;
          el.pause();
        })
        .catch(() => {});
    };

    const finishOk = (usedCors) => {
      try {
        if (full.el.readyState >= 2) full.el.currentTime = 0.01;
      } catch {
      ...
      full.ready = true;
      slot.fullReady = true;
      slot.fullTextureReady = usedCors;
      if (full.el.videoWidth && full.el.videoHeight) {
        uniforms.fullRes[index * 2] = full.el.videoWidth;
        uniforms.fullRes[index * 2 + 1] = full.el.videoHeight;
      }
    ...
    // Prefer CORS (WebGL texture). Vimeo progressive URLs often lack ACAO —
    // fall back to a non-CORS element so the HTML player can still play.
    applySrc(true);
    return waitForMediaReady(full.el)
      .then(() => finishOk(true))
      .catch(() => {
        applySrc(false);
        return waitForMediaReady(full.el).then(() => finishOk(false));
      });
  }
