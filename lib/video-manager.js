...
/** WebKit's canvas→GPU upload is far slower; keep cells smaller there. */
const SAFARI_CELL_W = 1280;
const SAFARI_CELL_H = 720;
const SAFARI_ATLAS_HZ = 20;

/** Safari / iOS WebKit (Chrome on iOS is WebKit too). */
export function isSlowVideoGpu() {
...
export function createVideoManager(container, fallbackTexture, uniforms) {
  ...
  const targetW = slowGpu ? SAFARI_CELL_W : TARGET_CELL_W;
  const targetH = slowGpu ? SAFARI_CELL_H : TARGET_CELL_H;
  const cellScale = Math.min(
    1,
    MAX_ATLAS_DIM / (atlasCols * targetW),
    MAX_ATLAS_DIM / (atlasRows * targetH),
  );
  const cellW = Math.max(1, Math.round(targetW * cellScale));
  const cellH = Math.max(1, Math.round(targetH * cellScale));
  const atlasMinInterval = slowGpu ? 1000 / SAFARI_ATLAS_HZ : 0;
  let lastAtlasMs = 0;
  ...
  atlasCtx.imageSmoothingQuality = slowGpu ? "low" : "high";
  ...
  function updateAtlas(now = performance.now()) {
    if (!atlasCtx) return;
    if (atlasMinInterval > 0 && now - lastAtlasMs < atlasMinInterval) return;
    lastAtlasMs = now;
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      atlasCtx.drawImage(el, col * cellW, row * cellH, cellW, cellH);
      drew = true;
    }
  ...
  /**
   * Keep decoding only the previews that are still on-screen.
