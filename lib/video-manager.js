...
export function createVideoManager(container, fallbackTexture, uniforms) {
  ...
  /** Pause / unload media for blobs that have been culled for performance. */
  function cullFrom(startIndex) {
    for (let i = Math.max(0, startIndex); i < slots.length; i++) {
      releaseFull(i);
      const preview = slots[i]?.preview;
      if (!preview || preview.kind !== "video") continue;
      const el = /** @type {HTMLVideoElement} */ (preview.el);
      el.pause();
      try {
